inherited ProF: TProF
  Left = 116
  Top = 83
  VertScrollBar.Range = 0
  BorderStyle = bsToolWindow
  Caption = #1575#1591#1604#1575#1593#1575#1578' '#1578#1576#1604#1610#1594#1575#1578
  ClientHeight = 584
  ClientWidth = 1007
  OldCreateOrder = True
  OnActivate = FormActivate
  OnDestroy = FormDestroy
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 1007
    DesignSize = (
      1007
      41)
    inherited LblForm: TLabel
      Left = 932
      Anchors = [akLeft, akTop, akRight]
    end
  end
  inherited Panel2: TPanel
    Tag = 5
    Top = 543
    Width = 1007
    object InsertPnl: TPanel
      Left = 751
      Top = 1
      Width = 255
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 1
      object BitBtn4: TBitBtn
        Left = 169
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetInsert1
        Caption = #1580#1583#1610#1583
        TabOrder = 0
      end
      object BitBtn5: TBitBtn
        Left = 88
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetEdit1
        Caption = #1608#1610#1585#1575#1610#1588
        TabOrder = 1
      end
      object BitBtn6: TBitBtn
        Left = 7
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetDelete1
        Caption = #1581#1584#1601
        TabOrder = 2
      end
    end
    object okPnl: TPanel
      Left = 583
      Top = 1
      Width = 168
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 2
      Visible = False
      object BitBtn7: TBitBtn
        Left = 85
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetPost1
        Caption = #1578#1575#1574#1610#1583
        TabOrder = 0
      end
      object BitBtn8: TBitBtn
        Left = 3
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetCancel1
        Caption = #1575#1606#1589#1585#1575#1601
        TabOrder = 1
      end
    end
    object BitBtn9: TBitBtn
      Left = 99
      Top = 8
      Width = 75
      Height = 25
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 3
      OnClick = BitBtn9Click
    end
    object BitBtn3: TBitBtn
      Left = 181
      Top = 8
      Width = 75
      Height = 25
      Caption = #1575#1585#1587#1575#1604' '#1576#1607
      TabOrder = 4
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        D29ED0EDD9FFFFFFFCFDFDFFFFFF}
    end
    object BitBtn11: TBitBtn
      Left = 262
      Top = 8
      Width = 75
      Height = 25
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 5
      OnClick = BitBtn11Click
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFF000000}
    end
    object BitBtn10: TBitBtn
      Left = 344
      Top = 8
      Width = 75
      Height = 25
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 6
      OnClick = BitBtn10Click
      Glyph.Data = {
        
        FFFFFEFEFEFFFFFFFFFFFF000000}
    end
  end
  object Panel4: TPanel
    Tag = 1
    Left = 0
    Top = 129
    Width = 1007
    Height = 136
    Align = alTop
    BevelInner = bvRaised
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 2
    DesignSize = (
      1003
      132)
    object Label4: TLabel
      Left = 929
      Top = 10
      Width = 45
      Height = 13
      Anchors = [akRight]
      Caption = #1705#1583' '#1588#1585#1705#1578
      FocusControl = DBEdit4
    end
    object Label5: TLabel
      Left = 929
      Top = 34
      Width = 47
      Height = 13
      Anchors = [akRight]
      Caption = #1606#1575#1605' '#1588#1585#1705#1578
      FocusControl = DBEdit5
    end
    object Label6: TLabel
      Left = 218
      Top = 34
      Width = 49
      Height = 13
      Caption = #1606#1608#1593' '#1601#1593#1575#1604#1610#1578
      FocusControl = DBEdit6
    end
    object Label7: TLabel
      Left = 218
      Top = 10
      Width = 86
      Height = 13
      Caption = #1705#1583' '#1606#1585#1605' '#1575#1601#1586#1575#1585#1610' '#1578#1604#1601#1606
      FocusControl = DBEdit7
    end
    object Label8: TLabel
      Left = 929
      Top = 59
      Width = 30
      Height = 13
      Anchors = [akRight]
      Caption = #1578#1604#1601#1606' 1'
    end
    object Label9: TLabel
      Left = 929
      Top = 84
      Width = 30
      Height = 13
      Anchors = [akRight]
      Caption = #1578#1604#1601#1606' 2'
    end
    object Label10: TLabel
      Left = 929
      Top = 108
      Width = 30
      Height = 13
      Anchors = [akRight]
      Caption = #1578#1604#1601#1606' 3'
    end
    object Label11: TLabel
      Left = 218
      Top = 59
      Width = 50
      Height = 13
      Caption = #1606#1575#1605' '#1605#1587#1574#1608#1604
      FocusControl = DBEdit11
    end
    object Label12: TLabel
      Left = 218
      Top = 84
      Width = 27
      Height = 13
      Caption = #1601#1575#1705#1587
    end
    object Label13: TLabel
      Left = 218
      Top = 108
      Width = 28
      Height = 13
      Caption = #1605#1608#1576#1575#1610#1604
    end
    object Label14: TLabel
      Left = 615
      Top = 59
      Width = 24
      Height = 13
      Anchors = [akRight]
      Caption = 'Email'
    end
    object SpeedButton2: TSpeedButton
      Left = 774
      Top = 5
      Width = 23
      Height = 22
      Anchors = [akTop, akRight]
      Caption = '...'
      Visible = False
      OnClick = SpeedButton2Click
    end
    object Label16: TLabel
      Left = 600
      Top = 84
      Width = 39
      Height = 13
      Anchors = [akRight]
      Caption = 'Website'
    end
    object DBEdit4: TDBEdit
      Tag = 1
      Left = 800
      Top = 6
      Width = 121
      Height = 21
      Anchors = [akRight]
      Color = clInfoBk
      DataField = 'CustomerID'
      DataSource = srcCustomers
      Enabled = False
      ReadOnly = True
      TabOrder = 0
      OnKeyDown = DBEdit4KeyDown
    end
    object DBEdit5: TDBEdit
      Tag = 2
      Left = 642
      Top = 30
      Width = 279
      Height = 21
      Anchors = [akRight]
      Color = clInfoBk
      DataField = 'CustomerName'
      DataSource = srcCustomers
      Enabled = False
      TabOrder = 1
    end
    object DBEdit6: TDBEdit
      Tag = 9
      Left = 32
      Top = 30
      Width = 182
      Height = 21
      Color = clBtnFace
      DataField = '__ActivityGroupName'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
    end
    object DBEdit7: TDBEdit
      Tag = 8
      Left = 136
      Top = 6
      Width = 78
      Height = 21
      Color = 14148607
      DataField = 'CustTel'
      DataSource = srcCustomers
      ReadOnly = True
      TabOrder = 7
    end
    object DBEdit8: TDBEdit
      Tag = 3
      Left = 800
      Top = 55
      Width = 121
      Height = 21
      Anchors = [akRight]
      Color = clBtnFace
      DataField = '_Tel1'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit9: TDBEdit
      Tag = 4
      Left = 800
      Top = 80
      Width = 121
      Height = 21
      Anchors = [akRight]
      Color = clBtnFace
      DataField = '_Tel2'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 3
    end
    object DBEdit10: TDBEdit
      Tag = 5
      Left = 800
      Top = 104
      Width = 121
      Height = 21
      Anchors = [akRight]
      Color = clBtnFace
      DataField = '_Tel3'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 4
    end
    object DBEdit11: TDBEdit
      Tag = 10
      Left = 32
      Top = 55
      Width = 182
      Height = 21
      Color = clBtnFace
      DataField = '_Family'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
    end
    object DBEdit12: TDBEdit
      Tag = 11
      Left = 93
      Top = 80
      Width = 121
      Height = 21
      Color = clBtnFace
      DataField = '_Fax'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 10
    end
    object DBEdit13: TDBEdit
      Tag = 12
      Left = 93
      Top = 104
      Width = 121
      Height = 21
      Color = clBtnFace
      DataField = '_Mobile'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 11
    end
    object DBEdit14: TDBEdit
      Tag = 6
      Left = 642
      Top = 55
      Width = 147
      Height = 21
      Anchors = [akRight]
      Color = clBtnFace
      DataField = '_Email'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBNavigator1: TDBNavigator
      Left = 646
      Top = 8
      Width = 120
      Height = 18
      DataSource = srcCustomers
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      Anchors = [akTop, akRight]
      TabOrder = 12
      Visible = False
    end
    object DBEdit16: TDBEdit
      Tag = 7
      Left = 642
      Top = 80
      Width = 147
      Height = 21
      Anchors = [akRight]
      Color = clBtnFace
      DataField = '_WebSite'
      DataSource = srcCustomers
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clActiveCaption
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 6
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 41
    Width = 1007
    Height = 88
    Align = alTop
    BevelInner = bvSpace
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 3
    DesignSize = (
      1003
      84)
    object Label1: TLabel
      Left = 928
      Top = 8
      Width = 50
      Height = 13
      Anchors = [akRight]
      Caption = #1588#1605#1575#1585#1607' '#1601#1585#1605
      FocusControl = DBEdit1
    end
    object Label2: TLabel
      Left = 928
      Top = 34
      Width = 48
      Height = 13
      Anchors = [akRight]
      Caption = #1578#1575#1585#1610#1582' '#1575#1585#1578#1576#1575#1591
    end
    object Label3: TLabel
      Left = 928
      Top = 59
      Width = 47
      Height = 13
      Anchors = [akRight]
      Caption = #1586#1605#1575#1606' '#1575#1585#1578#1576#1575#1591
    end
    object SpeedButton1: TSpeedButton
      Left = 826
      Top = 3
      Width = 23
      Height = 22
      Anchors = [akTop, akRight]
      Caption = '...'
      OnClick = SpeedButton1Click
    end
    object BitBtn2: TBitBtn
      Left = 40
      Top = 8
      Width = 75
      Height = 25
      Caption = #1601#1585#1575#1582#1608#1575#1606' '#1578#1604#1601#1606
      TabOrder = 0
      OnClick = BitBtn2Click
    end
    object DBEdit1: TDBEdit
      Left = 850
      Top = 4
      Width = 70
      Height = 21
      Anchors = [akTop, akRight]
      DataField = 'PropagandaID'
      DataSource = srcPropaganda
      TabOrder = 1
      OnKeyDown = DBEdit1KeyDown
    end
    object MskDate: TMaskEdit
      Left = 850
      Top = 30
      Width = 70
      Height = 21
      Anchors = [akTop, akRight]
      Color = 14548957
      EditMask = '!99/99/00;1;_'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      MaxLength = 8
      ParentFont = False
      TabOrder = 2
      Text = '  /  /  '
    end
    object MskTime: TMaskEdit
      Left = 850
      Top = 55
      Width = 73
      Height = 21
      Anchors = [akTop, akRight]
      EditMask = '!90:00;1;_'
      MaxLength = 5
      TabOrder = 3
      Text = '  :  '
    end
  end
  object Panel5: TPanel
    Tag = 2
    Left = 0
    Top = 265
    Width = 1007
    Height = 136
    Align = alTop
    BevelInner = bvRaised
    BevelOuter = bvLowered
    BorderWidth = 1
    BorderStyle = bsSingle
    TabOrder = 4
    DesignSize = (
      1003
      132)
    object Label15: TLabel
      Left = 829
      Top = 16
      Width = 78
      Height = 13
      Anchors = [akRight]
      Caption = #1606#1575#1605' '#1605#1582#1575#1591#1576' '#1578#1605#1575#1587
      FocusControl = DBEdit15
    end
    object Label17: TLabel
      Left = 297
      Top = 6
      Width = 57
      Height = 13
      Caption = #1606#1578#1610#1580#1607' '#1578#1605#1575#1587':'#8207
    end
    object DBMemo1: TDBMemo
      Tag = 2
      Left = 3
      Top = 3
      Width = 294
      Height = 126
      Align = alLeft
      BevelInner = bvLowered
      BevelOuter = bvSpace
      BevelKind = bkSoft
      BevelWidth = 3
      Color = clCream
      DataField = 'ProComment'
      DataSource = srcPropaganda
      TabOrder = 2
    end
    object DBEdit15: TDBEdit
      Left = 653
      Top = 34
      Width = 254
      Height = 21
      Anchors = [akRight]
      DataField = 'CallerName'
      DataSource = srcPropaganda
      TabOrder = 0
    end
    object DBRadioGroup1: TDBRadioGroup
      Tag = 1
      Left = 654
      Top = 64
      Width = 253
      Height = 49
      Anchors = [akRight]
      Caption = #1578#1575#1574#1610#1583#1607' '#1601#1575#1705#1587
      Columns = 2
      Ctl3D = True
      DataField = 'FaxDone'
      DataSource = srcPropaganda
      Items.Strings = (
        #1601#1575#1603#1587' '#1588#1583
        #1575#1585#1587#1575#1604' '#1606#1588#1583)
      ParentCtl3D = False
      TabOrder = 1
      Values.Strings = (
        '0'
        '1')
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 401
    Width = 1007
    Height = 142
    Align = alClient
    Color = clCream
    DataSource = srcPropaganda
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'PropagandaID'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ProDate'
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = '_CompanyName'
        Width = 149
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FaxDone'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CallerName'
        Width = 121
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ProComment'
        Width = 342
        Visible = True
      end>
  end
  object qryPropaganda: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    AfterInsert = qryPropagandaAfterInsert
    AfterEdit = qryPropagandaAfterEdit
    BeforePost = qryPropagandaBeforePost
    AfterPost = qryPropagandaAfterPost
    AfterCancel = qryPropagandaAfterCancel
    BeforeDelete = qryPropagandaBeforeDelete
    AfterDelete = qryPropagandaAfterDelete
    AfterScroll = qryPropagandaAfterScroll
    Parameters = <>
    SQL.Strings = (
      'select *'
      'From Propaganda')
    Left = 208
    Top = 24
    object qryPropagandaPropagandaID: TIntegerField
      DisplayLabel = #1588#1605#1575#1585#1607' '#1601#1585#1605
      FieldName = 'PropagandaID'
    end
    object qryPropagandaProComment: TStringField
      Tag = 3
      DisplayLabel = #1606#1578#1610#1580#1607' '#1578#1605#1575#1587
      FieldName = 'ProComment'
      Size = 300
    end
    object qryPropagandaFaxDone: TWordField
      DisplayLabel = #1606#1578#1610#1580#1607' '#1601#1575#1705#1587
      FieldName = 'FaxDone'
      Required = True
      OnGetText = qryPropagandaFaxDoneGetText
      OnSetText = qryPropagandaFaxDoneSetText
      MaxValue = 1
    end
    object qryPropagandaCallerName: TStringField
      Tag = 3
      DisplayLabel = #1606#1575#1605' '#1605#1582#1575#1591#1576' '#1578#1605#1575#1587
      FieldName = 'CallerName'
      Size = 50
    end
    object qryPropagandaCompanyID: TIntegerField
      DisplayLabel = #1603#1583' '#1588#1585#1603#1578
      FieldName = 'CompanyID'
    end
    object qryPropaganda_CompanyName: TStringField
      Tag = 3
      DisplayLabel = #1606#1575#1605' '#1588#1585#1603#1578
      FieldKind = fkLookup
      FieldName = '_CompanyName'
      LookupDataSet = qryCustomers
      LookupKeyFields = 'CustomerID'
      LookupResultField = 'CustomerName'
      KeyFields = 'CompanyID'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryPropagandaProTime: TStringField
      Tag = 3
      DisplayLabel = #1586#1605#1575#1606' '#1575#1585#1578#1576#1575#1591
      FieldName = 'ProTime'
      Size = 8
    end
    object qryPropagandaProDate: TStringField
      Tag = 3
      DisplayLabel = #1578#1575#1585#1610#1582' '#1575#1585#1578#1576#1575#1591
      FieldName = 'ProDate'
      Size = 8
    end
  end
  object qryCustomers: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      'from Customers'
      'where CustType=0 or  CustType=2')
    Left = 432
    Top = 16
    object qryCustomersCustomerID: TIntegerField
      FieldName = 'CustomerID'
    end
    object qryCustomersCustomerName: TStringField
      FieldName = 'CustomerName'
      Size = 50
    end
    object qryCustomersCustType: TIntegerField
      FieldName = 'CustType'
    end
    object qryCustomersCustTel: TIntegerField
      FieldName = 'CustTel'
    end
    object qryCustomers_Tel1: TStringField
      FieldKind = fkLookup
      FieldName = '_Tel1'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Tel1'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Tel2: TStringField
      FieldKind = fkLookup
      FieldName = '_Tel2'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Tel2'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Tel3: TStringField
      FieldKind = fkLookup
      FieldName = '_Tel3'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Tel3'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Fax: TStringField
      FieldKind = fkLookup
      FieldName = '_Fax'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Fax'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Mobile: TStringField
      FieldKind = fkLookup
      FieldName = '_Mobile'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Mobile'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Email: TStringField
      FieldKind = fkLookup
      FieldName = '_Email'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Email'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_WebSite: TStringField
      FieldKind = fkLookup
      FieldName = '_WebSite'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'WebSite'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
    object qryCustomers_Family: TStringField
      FieldKind = fkLookup
      FieldName = '_Family'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = 'Family'
      KeyFields = 'CustTel'
      Size = 50
      Lookup = True
    end
    object qryCustomers__ActivityGroupName: TStringField
      FieldKind = fkLookup
      FieldName = '__ActivityGroupName'
      LookupDataSet = qryTel
      LookupKeyFields = 'TelID'
      LookupResultField = '_ActivityGroupName'
      KeyFields = 'CustTel'
      LookupCache = True
      Size = 50
      Lookup = True
    end
  end
  object srcPropaganda: TDataSource
    DataSet = qryPropaganda
    OnStateChange = srcPropagandaStateChange
    Left = 208
    Top = 72
  end
  object srcCustomers: TDataSource
    DataSet = qryCustomers
    Left = 480
    Top = 73
  end
  object qryTel: TADOQuery
    Active = True
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      'from tel')
    Left = 328
    Top = 25
    object qryTelTel1: TStringField
      FieldName = 'Tel1'
      Size = 50
    end
    object qryTelTel2: TStringField
      FieldName = 'Tel2'
      Size = 50
    end
    object qryTelTel3: TStringField
      FieldName = 'Tel3'
      Size = 50
    end
    object qryTelFax: TStringField
      FieldName = 'Fax'
      Size = 50
    end
    object qryTelMobile: TStringField
      FieldName = 'Mobile'
      Size = 50
    end
    object qryTelEmail: TStringField
      FieldName = 'Email'
      Size = 50
    end
    object qryTelTelID: TIntegerField
      FieldName = 'TelID'
    end
    object qryTelName: TStringField
      FieldName = 'Name'
      Size = 50
    end
    object qryTelWebSite: TStringField
      FieldName = 'WebSite'
      Size = 50
    end
    object qryTelFamily: TStringField
      FieldName = 'Family'
      Size = 50
    end
    object qryTelActivityGroup: TIntegerField
      FieldName = 'ActivityGroup'
    end
    object qryTel_ActivityGroupName: TStringField
      FieldKind = fkLookup
      FieldName = '_ActivityGroupName'
      LookupDataSet = qryFormInfo
      LookupKeyFields = 'FormInfoID'
      LookupResultField = 'FormInfoName'
      KeyFields = 'ActivityGroup'
      LookupCache = True
      Size = 50
      Lookup = True
    end
  end
  object actPro: TActionList
    Left = 88
    Top = 8
    object DataSetInsert1: TDataSetInsert
      Category = 'Dataset'
      Caption = #1580#1583#1610#1583
      ShortCut = 113
      DataSource = srcPropaganda
    end
    object DataSetEdit1: TDataSetEdit
      Category = 'Dataset'
      Caption = #1608#1610#1585#1575#1610#1588
      ShortCut = 115
      DataSource = srcPropaganda
    end
    object DataSetDelete1: TDataSetDelete
      Category = 'Dataset'
      Caption = #1581#1584#1601
      ShortCut = 46
      DataSource = srcPropaganda
    end
    object DataSetPost1: TDataSetPost
      Category = 'Dataset'
      Caption = #1578#1575#1574#1610#1583
      ShortCut = 119
      DataSource = srcPropaganda
    end
    object DataSetCancel1: TDataSetCancel
      Category = 'Dataset'
      Caption = #1575#1606#1589#1585#1575#1601
      DataSource = srcPropaganda
    end
  end
  object qryFormInfo: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      'from FormInfo')
    Left = 600
    Top = 73
    object qryFormInfoFormInfoID: TIntegerField
      FieldName = 'FormInfoID'
    end
    object qryFormInfoFormInfoName: TStringField
      FieldName = 'FormInfoName'
      Size = 50
    end
  end
  object srcTel: TDataSource
    DataSet = qryTel
    Left = 328
    Top = 73
  end
end
