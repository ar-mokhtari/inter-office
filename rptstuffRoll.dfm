inherited rptstuffRollF: TrptstuffRollF
  Left = 324
  Top = 225
  Width = 944
  ActiveControl = DBGrid1
  Caption = #1711#1586#1575#1585#1588' '#1605#1575#1606#1583#1607' '#1605#1608#1580#1608#1583#1610' '#1575#1606#1576#1575#1585
  OldCreateOrder = True
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 928
    TabOrder = 1
    inherited LblForm: TLabel
      Left = -360
      Top = 24
      Width = 3
      Anchors = [akTop, akRight]
      Caption = ''
    end
    object RadioGroup1: TRadioGroup
      Left = 616
      Top = 1
      Width = 311
      Height = 39
      Align = alRight
      Caption = #1608#1590#1593#1610#1578
      Columns = 3
      Ctl3D = True
      ItemIndex = 2
      Items.Strings = (
        #1605#1608#1602#1578
        #1602#1591#1593#1610
        #1605#1608#1602#1578' '#1608' '#1602#1591#1593#1610)
      ParentCtl3D = False
      TabOrder = 0
      TabStop = True
      OnClick = RadioGroup1Click
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 1
      Width = 96
      Height = 39
      Align = alLeft
      Caption = #1585#1575#1607#1606#1605#1575#1610' '#1606#1605#1575#1610#1588
      TabOrder = 1
      object Label2: TLabel
        Left = 3
        Top = 19
        Width = 68
        Height = 13
        Caption = #1605#1608#1580#1608#1583#1610' '#1605#1606#1601#1610
      end
      object Shape2: TShape
        Left = 75
        Top = 20
        Width = 13
        Height = 14
        Brush.Color = 14019327
      end
    end
  end
  inherited Panel2: TPanel
    Width = 928
    TabOrder = 2
    object BitBtn8: TBitBtn
      Left = 95
      Top = 8
      Width = 75
      Height = 25
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 1
      OnClick = BitBtn8Click
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFF000000}
    end
    object BitBtn9: TBitBtn
      Left = 175
      Top = 8
      Width = 75
      Height = 25
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 2
      OnClick = BitBtn9Click
      Glyph.Data = {
        
        FFFFFEFEFEFFFFFFFFFFFF000000}
    end
    object BitBtn6: TBitBtn
      Left = 256
      Top = 8
      Width = 75
      Height = 25
      Caption = #1575#1585#1587#1575#1604' '#1576#1607
      TabOrder = 3
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        D29ED0EDD9FFFFFFFCFDFDFFFFFF}
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 41
    Width = 928
    Height = 339
    Hint = 
      #1576#1585#1575#1610' '#1580#1587#1578#1580#1608#1610' '#1587#1585#1610#1593#1548' '#1588#1605#1575#1585#1607' '#1705#1583' '#1705#1575#1604#1575#1610' '#1605#1608#1585#1583' '#1606#1592#1585' '#1585#1575' '#1576#1583#1608#1606' '#1608#1602#1601#1607' '#1578#1575#1610#1662' '#1606#1605#1575#1574 +
      #1610#1583
    Align = alClient
    Color = clCream
    Ctl3D = True
    DataSource = srcTemp
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit]
    ParentCtl3D = False
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnKeyPress = DBGrid1KeyPress
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 380
    Width = 928
    Height = 19
    Panels = <
      item
        Width = 50
      end>
  end
  object qryTemp: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      'from Remainder')
    Left = 112
    Top = 80
    object qryTempRcode: TIntegerField
      DisplayLabel = #1705#1583' '#1705#1575#1604#1575
      FieldName = 'Rcode'
    end
    object qryTempModelName: TWideStringField
      Tag = 3
      DisplayLabel = #1605#1583#1604' '#1705#1575#1604#1575
      FieldName = 'ModelName'
      Size = 255
    end
    object qryTempFirstInventory: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1602#1583#1575#1585' '#1575#1608#1604' '#1583#1608#1585#1607
      FieldName = 'FirstInventory'
      ReadOnly = True
    end
    object qryTempReceiptAmount: TIntegerField
      DisplayLabel = #1585#1587#1610#1583' '#1605#1608#1602#1578
      FieldName = 'ReceiptAmount'
      ReadOnly = True
    end
    object qryTempAmount: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1580#1605#1608#1593' '#1581#1608#1575#1604#1607' '#1607#1575#1610' '#1605#1608#1602#1578
      FieldName = 'Amount'
      ReadOnly = True
    end
    object qryTempResult: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1575#1606#1583#1607
      FieldName = 'Result'
      ReadOnly = True
    end
  end
  object srcTemp: TDataSource
    DataSet = qryTemp
    Left = 112
    Top = 128
  end
  object qryConfirm: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select *'
      'from Remainder1')
    Left = 216
    Top = 88
    object qryConfirmRcode: TIntegerField
      DisplayLabel = #1705#1583' '#1705#1575#1604#1575
      FieldName = 'Rcode'
    end
    object qryConfirmModelName: TWideStringField
      Tag = 3
      DisplayLabel = #1605#1583#1604' '#1705#1575#1604#1575
      FieldName = 'ModelName'
      Size = 255
    end
    object qryConfirmFirstInventory: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1602#1583#1575#1585' '#1575#1608#1604' '#1583#1608#1585#1607
      FieldName = 'FirstInventory'
      ReadOnly = True
    end
    object qryConfirmReceiptAmount: TIntegerField
      DisplayLabel = #1605#1580#1605#1608#1593' '#1585#1587#1610#1583#1607#1575#1610' '#1602#1591#1593#1610
      FieldName = 'ReceiptAmount'
      ReadOnly = True
    end
    object qryConfirmAmountSum1: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1580#1605#1608#1593' '#1581#1608#1575#1604#1607' '#1607#1575#1610' '#1602#1591#1593#1610
      FieldName = 'AmountSum1'
      ReadOnly = True
    end
    object qryConfirmResult: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1575#1606#1583#1607
      FieldName = 'Result'
      ReadOnly = True
    end
  end
  object qryBoth: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select * from Reminder_Both')
    Left = 320
    Top = 96
    object qryBothRcode: TIntegerField
      Tag = 3
      DisplayLabel = #1705#1583' '#1705#1575#1604#1575
      FieldName = 'Rcode'
    end
    object qryBothModelName: TWideStringField
      Tag = 3
      DisplayLabel = #1605#1583#1604' '#1705#1575#1604#1575
      FieldName = 'ModelName'
      Size = 255
    end
    object qryBothFirstInventory: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1602#1583#1575#1585' '#1575#1608#1604' '#1583#1608#1585#1607
      FieldName = 'FirstInventory'
      ReadOnly = True
    end
    object qryBothAmount: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1580#1605#1608#1593' '#1581#1608#1575#1604#1607' '#1607#1575#1610' '#1605#1608#1602#1578
      FieldName = 'Amount'
      ReadOnly = True
    end
    object qryBothAmountSum1: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1580#1605#1608#1593' '#1581#1608#1575#1604#1607' '#1607#1575#1610' '#1602#1591#1593#1610
      FieldName = 'AmountSum1'
      ReadOnly = True
    end
    object qryBothReceiptAmount0: TIntegerField
      DisplayLabel = #1585#1587#1610#1583' '#1605#1608#1602#1578
      FieldName = 'ReceiptAmount0'
      ReadOnly = True
    end
    object qryBothReceiptAmount1: TIntegerField
      DisplayLabel = #1585#1587#1610#1583' '#1602#1591#1593#1610
      FieldName = 'ReceiptAmount1'
      ReadOnly = True
    end
    object qryBothResult: TIntegerField
      Tag = 3
      DisplayLabel = #1605#1575#1606#1583#1607
      FieldName = 'Result'
      ReadOnly = True
    end
  end
  object srcConfirm: TDataSource
    DataSet = qryConfirm
    Left = 216
    Top = 136
  end
  object srcBoth: TDataSource
    DataSet = qryBoth
    Left = 336
    Top = 160
  end
end
