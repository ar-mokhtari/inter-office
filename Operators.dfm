inherited OperatorsF: TOperatorsF
  Left = 426
  Top = 157
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #1605#1606#1608#1610' '#1603#1575#1585#1576#1585#1575#1606
  ClientHeight = 447
  ClientWidth = 712
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel [0]
    Left = 661
    Top = 51
    Width = 44
    Height = 13
    Caption = #1603#1583' '#1603#1575#1585#1576#1585#1610
    FocusControl = DBEdit1
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel [1]
    Left = 659
    Top = 93
    Width = 46
    Height = 13
    Caption = #1606#1575#1605' '#1603#1575#1585#1576#1585#1610
    FocusControl = DBEdit2
  end
  object Label5: TLabel [2]
    Left = 596
    Top = 183
    Width = 108
    Height = 13
    Caption = #1570#1582#1585#1610#1606' '#1575#1578#1589#1575#1604' '#1576#1607' '#1587#1610#1587#1578#1605
    FocusControl = DBEdit4
    Visible = False
  end
  object Label3: TLabel [3]
    Left = 661
    Top = 137
    Width = 45
    Height = 13
    Caption = #1603#1604#1605#1607' '#1593#1576#1608#1585
    FocusControl = DBEdit3
  end
  inherited Panel1: TPanel
    Width = 712
  end
  inherited Panel2: TPanel
    Top = 406
    Width = 712
    object InsertPnl: TPanel
      Left = 456
      Top = 1
      Width = 255
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 1
      object BitBtn4: TBitBtn
        Left = 169
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetInsert1
        Caption = #1580#1583#1610#1583
        TabOrder = 0
      end
      object BitBtn5: TBitBtn
        Left = 88
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetEdit1
        Caption = #1608#1610#1585#1575#1610#1588
        TabOrder = 1
      end
      object BitBtn6: TBitBtn
        Left = 7
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetDelete1
        Caption = #1581#1584#1601
        TabOrder = 2
      end
    end
    object okPnl: TPanel
      Left = 288
      Top = 1
      Width = 168
      Height = 39
      Align = alRight
      BevelOuter = bvNone
      TabOrder = 2
      Visible = False
      object BitBtn7: TBitBtn
        Left = 85
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetPost1
        Caption = #1578#1575#1574#1610#1583
        TabOrder = 0
      end
      object BitBtn8: TBitBtn
        Left = 3
        Top = 6
        Width = 75
        Height = 25
        Action = DataSetCancel1
        Caption = #1575#1606#1589#1585#1575#1601
        TabOrder = 1
      end
    end
    object BitBtn3: TBitBtn
      Left = 97
      Top = 8
      Width = 75
      Height = 25
      Caption = #1575#1585#1587#1575#1604' '#1576#1607
      TabOrder = 3
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        D29ED0EDD9FFFFFFFCFDFDFFFFFF}
    end
    object BitBtn2: TBitBtn
      Left = 178
      Top = 8
      Width = 75
      Height = 25
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 4
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFF000000}
    end
    object BitBtn9: TBitBtn
      Left = 258
      Top = 8
      Width = 75
      Height = 25
      Caption = #1578#1585#1578#1610#1576
      TabOrder = 5
      OnClick = BitBtn9Click
      Glyph.Data = {
        
        FFFFFEFEFEFFFFFFFFFFFF000000}
    end
  end
  object DBEdit1: TDBEdit
    Left = 651
    Top = 69
    Width = 54
    Height = 21
    DataField = 'UserID'
    DataSource = srcUsers
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
  end
  object DBEdit2: TDBEdit
    Left = 563
    Top = 109
    Width = 142
    Height = 21
    DataField = 'UserName'
    DataSource = srcUsers
    TabOrder = 3
  end
  object Panel3: TPanel
    Left = 0
    Top = 41
    Width = 241
    Height = 365
    Align = alLeft
    TabOrder = 8
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 239
      Height = 182
      Align = alTop
      Caption = #1575#1605#1603#1575#1606#1575#1578' '#1587#1610#1587#1578#1605
      TabOrder = 0
      object TreeView1: TTreeView
        Left = 2
        Top = 15
        Width = 235
        Height = 165
        Align = alClient
        Color = clMoneyGreen
        Indent = 19
        TabOrder = 0
        Items.Data = {
          
          E3}
      end
    end
    object GroupBox3: TGroupBox
      Left = 1
      Top = 183
      Width = 239
      Height = 181
      Align = alClient
      Caption = #1575#1605#1603#1575#1606#1575#1578' '#1603#1575#1585#1576#1585
      TabOrder = 1
      object TreeView2: TTreeView
        Left = 2
        Top = 15
        Width = 235
        Height = 164
        Align = alClient
        Color = clInfoBk
        Indent = 19
        TabOrder = 0
      end
    end
  end
  object DBEdit4: TDBEdit
    Left = 564
    Top = 199
    Width = 142
    Height = 21
    DataField = 'LastUse'
    DataSource = srcUsers
    Enabled = False
    ReadOnly = True
    TabOrder = 5
    Visible = False
  end
  object GroupBox1: TGroupBox
    Left = 241
    Top = 224
    Width = 470
    Height = 184
    Caption = #1604#1610#1587#1578' '#1603#1575#1585#1576#1585#1575#1606
    TabOrder = 9
    object DBGrid1: TDBGrid
      Left = 2
      Top = 15
      Width = 466
      Height = 167
      Align = alClient
      Color = clCream
      DataSource = srcUsers
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'UserID'
          Width = 93
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UserName'
          Width = 196
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PeresentID'
          Width = 156
          Visible = True
        end>
    end
  end
  object DBEdit3: TDBEdit
    Left = 564
    Top = 153
    Width = 142
    Height = 21
    DataField = 'PassWord'
    DataSource = srcUsers
    PasswordChar = '*'
    TabOrder = 4
  end
  object DBRadioGroup1: TDBRadioGroup
    Left = 251
    Top = 51
    Width = 185
    Height = 168
    Hint = 
      #1583#1608': '#1608#1585#1608#1583' '#1575#1591#1604#1575#1593#1575#1578' '#1587#1607': '#1608#1610#1585#1575#1610#1588' '#1575#1591#1604#1575#1593#1575#1578' '#1579#1576#1578' '#1588#1583#1607' '#1582#1608#1583' '#1705#1575#1585#1576#1585' '#1670#1607#1575#1585': '#1608#1610#1585#1575 +
      #1610#1588' '#1575#1591#1604#1575#1593#1575#1578' '#1607#1605#1607' '#1705#1575#1585#1576#1585#1575#1606' '#1581#1578#1610' '#1576#1593#1583' '#1575#1586' '#1670#1575#1662' '#1662#1606#1580': '#1601#1593#1575#1604' '#1576#1608#1583#1606' '#1587#1608#1574#1610#1670' '#1601#1585#1605' '#1705 +
      #1583#1610#1606#1711' '#1585#1608#1610' '#1601#1585#1605' '#1575#1589#1604#1610#1548' '#1576#1575#1586' '#1588#1583#1606' '#1601#1585#1605' '#1705#1583#1610#1606#1711#1548' '#1608#1610#1585#1575#1610#1588' '#1602#1610#1605#1578' '#1608#1575#1581#1583' '#1583#1585' '#1662#1610#1588' '#1601#1575 +
      #1705#1578#1608#1585#1548' '#1606#1605#1575#1610#1588' '#1583#1705#1605#1607' '#1578#1594#1610#1610#1585#1575#1578' '#1582#1575#1606#1608#1575#1583#1607' '#1705#1583#1610#1606#1711' '#1583#1585' '#1601#1585#1605' '#1705#1583#1610#1606#1711' '#1575#1591#1604#1575#1593#1575#1578
    Caption = #1587#1591#1581' '#1583#1587#1578#1585#1587#1610' '#1575#1591#1604#1575#1593#1575#1578
    DataField = 'UserLevel'
    DataSource = srcUsers
    Items.Strings = (
      #1705#1575#1585#1576#1585' '#1711#1586#1575#1585#1588#1575#1578
      #1575#1662#1585#1575#1578#1608#1585' '#1608#1585#1608#1583' '#1575#1591#1604#1575#1593#1575#1578
      #1575#1662#1585#1575#1578#1608#1585' '#1608#1585#1608#1583' '#1608' '#1575#1589#1604#1575#1581' '#1575#1591#1604#1575#1593#1575#1578
      #1587#1608#1662#1585#1608#1575#1610#1586#1585' '#1575#1591#1604#1575#1593#1575#1578
      #1605#1583#1610#1585#1610#1578' '#1591#1604#1575#1610#1610
      #1605#1583#1610#1585#1610#1578' '#1662#1604#1575#1578#1610#1606#1610)
    ParentShowHint = False
    ShowHint = True
    TabOrder = 7
    Visible = False
  end
  object DBRadioGroup2: TDBRadioGroup
    Left = 440
    Top = 51
    Width = 104
    Height = 65
    Caption = #1606#1608#1593' '#1575#1587#1578#1601#1575#1583#1607' '
    DataField = 'UserGrade'
    DataSource = srcUsers
    Items.Strings = (
      #1705#1575#1585#1576#1585' '#1662#1610#1588' '#1601#1575#1705#1578#1608#1585
      #1705#1575#1585#1576#1585' '#1575#1606#1576#1575#1585)
    TabOrder = 6
    Visible = False
  end
  object DBRadioGroup3: TDBRadioGroup
    Left = 440
    Top = 154
    Width = 104
    Height = 65
    Caption = #1605#1581#1604' '#1705#1575#1585
    DataField = 'UserPosition'
    DataSource = srcUsers
    Items.Strings = (
      #1601#1606#1610' '#1605#1607#1606#1583#1587#1610
      #1605#1583#1610#1585#1610#1578)
    TabOrder = 10
    Visible = False
  end
  object DBRadioGroup4: TDBRadioGroup
    Left = 563
    Top = 51
    Width = 79
    Height = 53
    Caption = #1583#1587#1578#1585#1587#1610
    DataField = 'UserAccess'
    DataSource = srcUsers
    Items.Strings = (
      #1601#1593#1575#1604
      #1594#1610#1585' '#1601#1593#1575#1604)
    TabOrder = 11
    Values.Strings = (
      '1'
      '0')
    Visible = False
  end
  object qryUsers: TADOQuery
    Connection = DmF.adoMain
    CursorType = ctStatic
    AfterInsert = qryUsersAfterInsert
    BeforeEdit = qryUsersBeforeEdit
    AfterEdit = qryUsersAfterEdit
    AfterPost = qryUsersAfterPost
    AfterCancel = qryUsersAfterCancel
    BeforeDelete = qryUsersBeforeDelete
    AfterDelete = qryUsersAfterDelete
    Parameters = <>
    SQL.Strings = (
      'SELECT     *'
      'FROM         Users')
    Left = 376
    Top = 296
    object qryUsersUserID: TIntegerField
      DisplayLabel = #1603#1583' '#1603#1575#1585#1576#1585#1610
      FieldName = 'UserID'
    end
    object qryUsersUserName: TStringField
      DisplayLabel = #1606#1575#1605' '#1603#1575#1585#1576#1585#1610
      FieldName = 'UserName'
      Required = True
      Size = 50
    end
    object qryUsersPassWord: TStringField
      DisplayLabel = #1603#1604#1605#1607' '#1593#1576#1608#1585
      FieldName = 'PassWord'
      Required = True
      Size = 50
    end
    object qryUsersUserGrade: TWordField
      DisplayLabel = #1587#1591#1581' '#1603#1575#1585#1576#1585#1610
      FieldName = 'UserGrade'
      Required = True
      OnGetText = qryUsersUserGradeGetText
      OnSetText = qryUsersUserGradeSetText
    end
    object qryUsersUserAccess: TStringField
      DisplayLabel = #1583#1587#1578#1585#1587#1610' '#1603#1575#1585#1576#1585#1610
      FieldName = 'UserAccess'
      Size = 250
    end
    object qryUsersLastUse: TStringField
      DisplayLabel = #1570#1582#1585#1610#1606' '#1575#1578#1589#1575#1604' '#1576#1607' '#1587#1610#1587#1578#1605
      FieldName = 'LastUse'
      Size = 50
    end
    object qryUsersPeresentID: TIntegerField
      DisplayLabel = #1605#1593#1585#1601
      FieldName = 'PeresentID'
      OnGetText = qryUsersPeresentIDGetText
    end
    object qryUsersUserLevel: TIntegerField
      DisplayLabel = #1587#1591#1581' '#1583#1587#1578#1585#1587#1610
      FieldName = 'UserLevel'
      Required = True
      OnGetText = qryUsersUserLevelGetText
      OnSetText = qryUsersUserLevelSetText
    end
    object qryUsersUserPosition: TWordField
      DisplayLabel = #1605#1581#1604' '#1705#1575#1585
      FieldName = 'UserPosition'
      Required = True
      OnGetText = qryUsersUserPositionGetText
      OnSetText = qryUsersUserPositionSetText
    end
  end
  object srcUsers: TDataSource
    DataSet = qryUsers
    OnStateChange = srcUsersStateChange
    Left = 328
    Top = 312
  end
  object ActionList1: TActionList
    Left = 448
    Top = 296
    object DataSetInsert1: TDataSetInsert
      Category = 'Dataset'
      Caption = #1580#1583#1610#1583
      DataSource = srcUsers
    end
    object DataSetDelete1: TDataSetDelete
      Category = 'Dataset'
      Caption = #1581#1584#1601
      DataSource = srcUsers
    end
    object DataSetEdit1: TDataSetEdit
      Category = 'Dataset'
      Caption = #1608#1610#1585#1575#1610#1588
      DataSource = srcUsers
    end
    object DataSetPost1: TDataSetPost
      Category = 'Dataset'
      Caption = #1578#1575#1574#1610#1583
      DataSource = srcUsers
    end
    object DataSetCancel1: TDataSetCancel
      Category = 'Dataset'
      Caption = #1575#1606#1589#1585#1575#1601
      DataSource = srcUsers
    end
  end
end
