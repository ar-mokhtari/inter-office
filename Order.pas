...
begin
  inherited;
   ...
   DetailPanel.Visible:=True;
   DetailPanel2.Visible:=True;
   UserLog(qryOrder,dsaInsert,'—”Ìœ ò«·«');
...
begin
  inherited;
  UserLog(qryOrderItems,dsaDel,'«ﬁ·«„ —”Ìœ ò«·«');
end;
...
begin
  inherited;
   ...
   UserLog(qryOrderItems,dsaEdit,'«ﬁ·«„ —”Ìœ ò«·«');
end;
...
begin
  inherited;
    ...
    s:=searchCode_ADOF.SearchCode2(DmF.adoMain,'Ã” ÃÊÌ ÕÊ«·Â',txt,['‘„«—Â ÕÊ«·Â','‰«„ „‘ —Ì'],result,[50,150],alLeft);
    if s then begin
...
begin
  inherited;
   if dbedit4.Text<>''then lblLetters.Caption:=num2alphabet(StrToInt64(RemoveComma(DBEdit4.Text)))+' —Ì«· '
   else lblLetters.Caption:='’›— —Ì«·';
end;
...
begin
  inherited;
  if DBText5.Caption='' then begin Warn('·ÿ›« ò«·« —« «‰ Œ«» ‰„«∆Ìœ');
                                Abort;
                          end;//if
  if DBEdit8.Text='' then begin Warn('·ÿ›« „ﬁœ«— —« Ê«—œ ‰„«∆Ìœ');
                                Abort;
                          end;//if
  if DBEdit7.Text='' then begin Warn('·ÿ›« ﬁÌ„  Ê«Õœ —« Ê«—œ ‰„«∆Ìœ');
                                Abort;
                          end;//if
end;
...
begin
  inherited;
    ...
    s:=searchCode_ADOF.SearchCode2(DmF.adoMain,'Ã” ÃÊÌ ÅÌ‘ ›«ò Ê—',txt,['‘„«—Â',' «—ÌŒ','ò«—‘‰«”','„»·€','‰«„ „‘ —Ì'],result,[50,70,100,90,150],alLeft);
    if s then
    begin
     try
       With qry do
       begin
        ...
        begin
          ...
          if qryReminder.FieldByName('Result').AsInteger<0 then (
            if get_response('„ÊÃÊœÌ ò«·«Ì ˛'+
                                                                     qryReminder.FieldByName('ModelName').AsString+
                                                                     'œ— Õ«· Õ«÷— „‰›Ì «” . ò«·« »Â «ﬁ·«„ ÕÊ«·Â «÷«›Â ‘Êœø ') <>mrYes
            then Next);
          qryOrderItems.Insert;
